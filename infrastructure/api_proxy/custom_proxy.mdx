...
1. **Use a custom endpoint name** specific to your product (e.g., `/v1/my-product-analytics` instead of `/v1/log_event`)
...
You can use the following javascript code snippet in your Lambda@Edge function. This example rewrites a custom endpoint `/v1/my-product-analytics` to the Statsig endpoint `/v1/log_event`:
...
export const handler = async (event, context, callback) => {
  ...
  // Replace 'my-product-analytics' with your custom endpoint name
  if (request.uri.includes('/v1/my-product-analytics')) {
    request.uri = request.uri.replace('/v1/my-product-analytics', '/v1/log_event');
  }
...
5.  Copy and paste the following snippet into the `worker.js` file, then hit "Deploy". This example rewrites a custom endpoint `/v1/my-product-analytics` to the Statsig endpoint `/v1/log_event`:
    ...
    export default {
      async fetch(request, _env, _ctx) {
        ...
        if (pathname.includes('/v1/my-product-analytics')) {
          pathname = pathname.replace('/v1/my-product-analytics', '/v1/log_event');
        }
...
For example, if your proxy rewrites `/v1/my-product-analytics` to `/v1/log_event`, you would configure:

```typescript
Statsig.initialize(mySdkKey, myUser, {
  networkConfig: {
    api: "https://my-statsig-proxy.com/v1",
    logEventUrl: "https://my-statsig-proxy.com/v1/my-product-analytics",
  },
});
```
