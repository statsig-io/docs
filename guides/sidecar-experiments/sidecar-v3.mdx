...
### Testing a specific user with overrides

If you want to QA a specific Visual Editor variant before launch, add an ID override for a known test user in the experiment's **Manage Overrides** flow. For example, you can create a user ID like `preview_test` and map it to the variant you want to inspect.

Then open your site with that same user ID in the query string:

`https://www.example.com/?overrideuser=preview_test`

This gives you a repeatable way to preview how that exact test user will experience the page. For teams testing multiple variants, it can be helpful to create a few dedicated IDs such as `preview_control`, `preview_test_a`, and `preview_test_b`.

## Advanced topics

If you need more advanced Sidecar controls after the basic setup is working:

- See [Advanced Configurations](/guides/sidecar-experiments/advanced-configurations-v3) for script URL tuning, runtime options, consent, and identity configuration.
