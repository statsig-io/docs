...
This example uses `window.postExperimentCallback` only as a one-time readiness hook so the GTM listener starts after Sidecar finishes its initial run. If you need one callback per activated experiment assignment for outbound analytics, use `window.statsigSidecarConfig.onExperimentEvaluation` instead.
...
window.StatsigLogger = (function () {
  ...
  var checkStatsigReady = function (callback) {
    ...
    if(typeof StatsigSidecar !== 'undefined' && StatsigSidecar.getStatsigInstance()) {
      ...
      // postExperimentCallback is used here as a one-time "Sidecar is ready" hook
