...
Use runtime options to control Sidecar behavior for cases like consent management, GDPR compliance, and logging behavior. All of the [StatsigOptions](/client/javascript-sdk/#statsig-options) provided by the JavaScript SDK are also fully supported with Sidecar. These can be passed to Sidecar via:
```js
...
### Managing Consent
Consent management is a common runtime-options use case.

Prior to the Sidecar script tag, configure these runtime options to disable browser storage and tracking:
