---
title:  Prefetchusersconfig
description: <CodeGroup> ```typescript TypeScript import type { StatsigUser } from "statsig-js"; const prefetchUsers: StatsigUser[] = [ { userID: "user_a", customI
---

<CodeGroup>

```typescript TypeScript
import type { StatsigUser } from "statsig-js";

const prefetchUsers: StatsigUser[] = [
  { userID: "user_a", customIDs: { groupID: "group_a" } },
  { userID: "user_b", customIDs: { groupID: "group_a" } },
];

await Statsig.initialize(
  "client-sdk-key",
  { userID: "user_a" },
  { prefetchUsers: prefetchUsers }
);
```text

```javascript JavaScript
const prefetchUsers = [
  { userID: "user_a", customIDs: { groupID: "group_a" } },
  { userID: "user_b", customIDs: { groupID: "group_a" } },
];

await Statsig.initialize(
  "client-sdk-key",
  { userID: "user_a" },
  { prefetchUsers: prefetchUsers }
);
```

</CodeGroup>
