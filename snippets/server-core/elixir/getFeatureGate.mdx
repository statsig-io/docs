...
description: ```elixir {:ok, feature_gate} = Statsig.get_feature_gate(user, "example_gate") options = %Statsig.FeatureGateEvaluationOptions{ disable_exposure_loggi
---
```elixir
{:ok, feature_gate} = Statsig.get_feature_gate(user, "example_gate")
# access the value, or the name off of the feature_gate object

This page explains access the value, or the name off of the feature_gate object.
