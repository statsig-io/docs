---
title: Stableid
description: Stable ID provides a consistent device identifier. It lets you run [logged-out experiments](/guides/first-device-level-experiment) and target gates at
---
...
- Local storage is scoped per domain, so cross-domain usage requires sharing the value manually (refer to the following example).
