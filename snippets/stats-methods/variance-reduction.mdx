...
- We also use stratification with CUPED to handle users with missing pre-experiment data, as described in the paper. Users are grouped into strata using pre-experiment information, treatment and control estimates are aggregated across strata within each group, and the usual difference-in-means and variance estimation is then applied.
