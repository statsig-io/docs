...
<Frame>
  <img src="/images/metric_hover_card.png" alt="UI for metric hover card in experiments" />
</Frame>
...
Reverse power is the smallest true effect size that an experiment can reliably detect as of now. It is determined by the realized sample size and the estimated standard error from the control group. Some studies refer to this value as the ex-post MDE (minimal detectable effect). Importantly, reverse power does *not* depend on the estimated effect size. In practice, reverse power answers such question: given how the test actually played out, what is the smallest effect we have 1-β (typically 0.8) to detect?

For a two-sided test, the reverse power for a given metric X is computed using the following equation:

$$
Reverse Power = \frac{(Z_{1-\beta} + Z_{1-\alpha/2})}{\bar{X_{\text{control}}}}\times \sqrt{\frac{\mathrm{var}(X)}{N_{\text{control}}}} \times 100\%
$$

For a one-sided test, the reverse power for a given metric X is computed using the following equation:

$$
Reverse Power = \frac{(Z_{1-\beta} + Z_{1-\alpha})}{\bar{X_{\text{control}}}}\times \sqrt{\frac{\mathrm{var}(X)}{N_{\text{control}}}} \times 100\%
$$

* *X<sub>control</sub>-bar* is the mean metric value across control users
* *var(X)* is the population variance of the control group
* *N<sub>control</sub>* are the observed number of units in the control group
* *Z<sub>1-&beta;</sub>* is the standard Z-score for the selected power.  Typically *1-&beta; = 0.8* and *Z<sub>1-&beta;</sub> = 0.84*
* *Z<sub>1-&alpha;/2</sub>* and *Z<sub>1-&alpha;</sub>* are the standard Z-scores for the selected significance level in a two-sided test and in a one-sided test. 

You can enable reverse power as an optional feature. To manage it, go to Settings -> Product Configuration -> Experimentation -> Organization, where you can toggle reverse power on or off. 
...
<Frame>
  <img src="/images/snippets/pulse/metric-drill-down/f42453a6-e413-47d5-b176-bb3e9b49a915.png" alt="Daily metric impact visualization interface" />
</Frame>
...
<Frame>
  <img src="/images/snippets/pulse/metric-drill-down/b2e9aea9-98fb-4e29-bccf-abae356d0173.png" alt="Cumulative metric lift visualization interface" />
</Frame>
...
<Frame>
  <img src="/images/snippets/pulse/metric-drill-down/17eeb054-43d8-424c-b43a-85c682bfcfb8.png" alt="Days since exposure metric visualization interface" />
</Frame>
...
<Frame>
  <img src="/images/snippets/pulse/metric-drill-down/171754168-624f740e-bdd6-46ea-a2e3-8d70e637ff75.png" alt="Experiment impact metrics interface" />
</Frame>
