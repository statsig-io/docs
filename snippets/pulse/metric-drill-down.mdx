...
MDE = \frac{(Z_{1-\beta} + Z_{1-\alpha/2})}{\bar{X_{\text{control}}}}\times \sqrt{\frac{\mathrm{var}(X)}{N_{\text{control}}}} \times 100\%

For a one-sided test, the reverse power for a given metric X is computed using the following equation:

MDE = \frac{(Z_{1-\beta} + Z_{1-\alpha})}{\bar{X_{\text{control}}}}\times \sqrt{\frac{\mathrm{var}(X)}{N_{\text{control}}}} \times 100\%
...
You can enable reverse power as an optioinal feature. To manage it, go to Settings -> Product Configuration -> Experimentation -> Organization, where you can toggle reverse power on or off. 
