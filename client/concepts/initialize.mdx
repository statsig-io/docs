...
- [**Asynchronous Initialization (Awaited)**](#asynchronous-initialization-awaited): Wait for the initialization network call to finish before rendering content.
- [**Bootstrap Initialization**](#bootstrap-initialization): Generate the assignment values on your own server, and pass them down with other request, resulting in zero-latency rendering. Best of both worlds for latency and availability of fresh assignments, but requires additional engineering effort.
- [**Asynchronous Initialization (Not Awaited)**](#asynchronous-initialization-not-awaited): Do not await the return of the initialization network call. This ensures immediate rendering, but in a state that reflects stale assignments or no assignments available.
- [**Synchronous Initialization**](#synchronous-initialization): Renders immediately, but with stale or no assignments available. First-visit users will never be assigned to gates and experiments.
...
### 1. Asynchronous Initialization - Awaited {#asynchronous-initialization-awaited}
...
### 3. Asynchronous Initialization - Not Awaited {#asynchronous-initialization-not-awaited}
...
### 4. Synchronous Initialization {#synchronous-initialization}
