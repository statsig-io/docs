...
description: "Reference for advanced experiment options in Statsig Warehouse Native, including variance reduction, CUPED, sequential testing, and stratified sampling."
keywords:
  - owner:craig
last_update:
  date: 2025-09-18
...
Explore queries enable drilldown, filtering, grouping, and more advanced ways to cut metric data by user and metric properties. These are a way to deeply understand experiment results. On Statsig, explore queries run with the same statistics as the scorecard; with no filters or other settings, results should match the scorecard results.
