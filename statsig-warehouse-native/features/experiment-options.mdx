...
<Note>
Statsig's [power analysis tools](/experiments-plus/power-analysis) are the best way to determine what the target duration should be. A power analysis can be attached to an experiment to add context on the duration.
</Note>
...
Configures multiple-comparisons corrections - either controlling the false positive rate or false discovery rate. Refer to the more detailed documentation for [Bonferroni](/stats-engine/methodologies/bonferroni) and [Benjamini-Hochberg](/statsig-warehouse-native/features/statistics/methodologies/benjamini-hochberg-procedure.mdx).
