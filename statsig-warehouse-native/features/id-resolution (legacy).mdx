---
title: ID Resolution (legacy)
sidebarTitle: ID Resolution (legacy)
description: Legacy reference for ID resolution in Statsig Warehouse Native, mapping cross-platform identifiers to analyze anonymous and logged-in users in experiments.
keywords:
  - owner:vm
last_update:
  date: 2025-10-21
...
<Info>
  We are interested in supporting more complex 1-to-many relationships of
  identities and are eager to partner with customers to develop these
  capabilities if a more advanced use-case is required.
</Info>
