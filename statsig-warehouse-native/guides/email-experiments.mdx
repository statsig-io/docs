...
description: "Run email experiments in Statsig Warehouse Native by mapping send and open events from your warehouse to assignment sources and metric definitions."
...
You should consider which down-funnel metrics should be included as primary metrics for making on a decision on your experiment. Attributing down-funnel interactions that occur prior to user login may be tricky, but can be solved by setting up [ID resolution in Statsig](/statsig-warehouse-native/features/id-resolution/). 
