---
title: Scaling Warehouses (β)
sidebarTitle: Scaling Warehouses β
keywords:
  - owner:vm
last_update:
  date: 2025-09-18
description: You can now connect multiple Snowflake warehouses to your account, enabling better query performance by automatically distributing query jobs across a
---


You can now connect multiple Snowflake warehouses to your account, enabling better query performance by automatically distributing query jobs across all available warehouses. To set it up, you can head over to Settings > Project > Data Connection, and select Set up additional Warehouses.
<Frame>
  <img src="/images/statsig-warehouse-native/guides/scaling/d3d4d05a-7627-4b0e-9ba7-aa4e6fe561e4.png" alt="Snowflake warehouse scaling configuration interface" />
</Frame>

This is a beta feature, only available on Snowflake for now.
