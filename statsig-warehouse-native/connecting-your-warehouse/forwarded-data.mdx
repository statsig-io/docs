...
Fast-forwarded exposures will be copied sent without deduplication with respect to the table kept in your warehouse. Additionally, statsig only holds onto exposure data for 30 days for warehouse native projects for privacy reasons; this means Statsig's server-side deduplication will start re-counting users after 30 days since they are "new to Statsig". These will be correctly deduplicated during the analysis in your warehouse.

This means that you should not expect statsig_forwarded_exposures to be unique or deduplicated for any pipeline use cases within your own data stack; you should deduplicate at the unit/experiment level. 
