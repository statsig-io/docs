...
Because Statsig only holds onto exposure data for 30 days, it cannot dedupe beyond that time on its servers. This means that after 30 days, the "deduplicated" data from Statsig will start to re-send users. These will be correctly deduplicated during analysis in your warehouse, but it means that you should not expect the table to have unique user records, even after filtering out the fast-forwarded exposures (which can also have duplicate records). 
