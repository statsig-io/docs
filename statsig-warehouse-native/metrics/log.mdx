...
title: Log Metrics
sidebarTitle: Log
description: Log metrics apply a logarithm to unit-level Sum or Count values before computing Pulse results, reducing skew. Base defaults to natural log and is configurable.
keywords:
  - owner:vm
last_update:
  date: 2025-09-18
...
This does mean there's potentially a confounding factor of participation rate; to mitigate this, results are presented the same as for [ratio](/statsig-warehouse-native/metrics/ratio) metrics and present statistics for the overall result as well as the implicit numerators and denominators.
