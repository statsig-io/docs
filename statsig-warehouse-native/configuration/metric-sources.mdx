...
- Raw event logging (event level data), using the log timestamp as the timestamp, example:
...
*- Number of users with purchase: You can create a [UNIT COUNT](https://docs.statsig.com/statsig-warehouse-native/metrics/unit-count-once), with a filter of event_name = 'purchase'.*
*- % of users with page view who clicked: You can create a [RATIO](https://docs.statsig.com/statsig-warehouse-native/metrics/ratio) with this metric source in both denominator and numerator, and apply event_name filter accordingly.*

- Fact tables (one row per entity per day), using the date of the row as the timestamp, example:
...
*Metric examples with this source:*
*- Revenue: You can create a [SUM](https://docs.statsig.com/statsig-warehouse-native/metrics/sum) metric on 'revenue' column, with any filters you need.*
*- Average order value: You can create a [MEAN](https://docs.statsig.com/statsig-warehouse-native/metrics/mean) metric on 'revenue' column, with any filters you need.*

- Aggregated fact tables at unit day granularity, using the date of the row as the timestamp, example: 
...
*Metric examples with this source:*
*- Revenue: You can create a [SUM](https://docs.statsig.com/statsig-warehouse-native/metrics/sum) metric on 'revenue' column, with any filters you need.*
*- Number of users with purchase: You can create a [UNIT COUNT](https://docs.statsig.com/statsig-warehouse-native/metrics/unit-count-once) metric, with a filter of purchase > 0.*

### Types of data that needs some transformation
There are times when you might have a table that does not fall into the schema shared above. For example, a wide user dimension table contains one row per user with pre-aggregated or derived behavioral attributes.

| `user_id` | `signup_date` | `country` | `platform` | `first_active_date` | `last_active_date` | `total_sessions` | `total_orders` | `lifetime_revenue` | `is_power_user` |
|-----------|---------------|-----------|------------|---------------------|--------------------|------------------|----------------|--------------------|-----------------|
| `u_123`   | 2023-11-12    | `US`      | `web`      | 2023-11-12          | 2024-03-02         | 58               | 7              | 249.85             | `true`          |
| `u_456`   | 2024-01-05    | `CA`      | `ios`      | 2024-01-06          | 2024-02-18         | 14               | 1              | 19.99              | `false`         |
| `u_789`   | 2024-02-10    | `GB`      | `android`  | 2024-02-10          | 2024-03-01         | 31               | 3              | 89.97              | `false`         |
