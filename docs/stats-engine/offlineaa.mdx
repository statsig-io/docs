...
A/A tests can be Online or Offline. An [Online A/A test](https://docs.statsig.com/guides/aa-test) is run on real users. An engineer instruments your app with the Statsig SDK to check for experiment assignment. Assignment is logged, but there's no difference in experience to the user.
