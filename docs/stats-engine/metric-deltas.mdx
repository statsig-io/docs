...
We define all deltas here to be the difference between a "treatment" group as compared to a presumably unchaged "control" group. However, Statsig enables comparison between any two groups as desired.
:::
...
For custom **event_user** metrics with "One-Time Event" selected, statsig computes how many users have the event at any time after the user has been in the experiment. This result is not normalized by the number of days a user is in the expeirment. The group mean is given by:
