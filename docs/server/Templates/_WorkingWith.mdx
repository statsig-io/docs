...
Now that your SDK is initialized, let's fetch a [**Feature Gate**](/feature-gates/overview). Feature Gates can be used to create logic branches in code that can be rolled out to different users from the Statsig Console. Gates are always **CLOSED** or **OFF** (think `return false;`) by default.
