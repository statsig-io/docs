...
The SDK implements `IDisposable` for proper resource cleanup:

```csharp
using var statsig = new Statsig("server-secret-key");
await statsig.Initialize();
```

Always dispose of Statsig instances to free native resources.
...
## Migration from Legacy .NET SDK

Key differences from the legacy .NET SDK:

- **Instance-based API** instead of static methods (`new Statsig()` vs `StatsigServer.Initialize()`)
- **Builder pattern** for user creation (`StatsigUserBuilder` vs direct object construction)
- **Enhanced type safety** with generic `Get<T>()` methods
- **Additional data types** like `ParameterStore` and `Layer`
- **Performance improvements** through Rust core integration
