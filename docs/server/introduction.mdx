...
1. Initialization
2. Evaluating Variants
3. Recording Exposure Checks & Custom Events

![Server SDKs](https://user-images.githubusercontent.com/1315028/169563936-121cb75e-37e5-491f-8bb1-2d853c2e04b8.png)

### 1. Initialization
-	The server SDK's `initialize` call takes the **server SDK key** and makes a network request to Statsig servers; this network call fetches all rule sets to your application server to enable local evaluation in the next phase

### 2. Evaluating Variants
-	The server SDK's `getExperiment` call takes a [**StatsigUser**](/server/concepts/user) object with a user identifier, evaluates the rules for the specified user, and returns the configuration of the variant that the user is assigned to; use the returned `config` object to `get` the value of the parameters required to implement the experiment variant

-	 If your experiment is part of a Layer, the `getLayer` call takes a [**StatsigUser**](/server/concepts/user) object with a user identifier and returns the experiment configuration for the variant that the your end-user is assigned to; use the returned `config` object to `get` the value of the parameters required to implement the experiment variant
...
- The server SDK automatically flushes all accumulated exposure checks and logged custom events to the Statsig servers every 60 seconds
