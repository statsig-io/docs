...
const paramValue = $layer->get('a_param_name', 'fallback_value');`}
  ...
  parameterStoreSnippet: (
    <CodeBlock language="php">
      {`$parameterStore = $this->statsig->getParameterStoreWithExposureLoggingDisabled($user, 'a_parameter_store_name');
$value = $parameterStore->getString('a_parameter_name', 'default_value');`}
    </CodeBlock>
  ),
};
