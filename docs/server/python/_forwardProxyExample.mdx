...
:::warning
GRPC Streaming requires python 3.8+. 
Do not enable ProxyConfig options if you are on an older version of python or your sdk may not operate correctly
::: 
...
```
When SDK disconnected from forward proxy when use grpc_websocket, sdk will retry connection with exponenetial backoff, after `push_worker_failover_threshold` retries, sdk will start polling from Statsig until reconnet to forward proxy.
