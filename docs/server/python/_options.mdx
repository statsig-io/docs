...
- **timeout**: `timeout: int = None`
  - Enforces a minimum timeout on network requests from the SDK (unit = seconds)
- **init_timeout**: `init_timeout: int = None`
  - Sets the maximum timeout on download config specs and id lists network requests for initialization (unit = seconds)
