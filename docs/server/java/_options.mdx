...
- **proxyConfig** - ProxyConfig
  ```kotlin
    ...
    * @property proxyProtocol The protocol scheme used by the proxy server. Defaults to "https".
    */
  ...
  **Example Usage**
  ...
  ```java
    ...
    ProxyConfig proxyConfigWithAuth = new ProxyConfig();
    proxyConfigWithAuth.setProxyHost("localhost");
    proxyConfigWithAuth.setProxyPort(8080);
    proxyConfigWithAuth.setProxyAuth(Credentials.basic("username", "password"));
    proxyConfigWithAuth.setProxyProtocol("https");
    ...
    StatsigOptions options = new StatsigOptions.Builder().setProxyConfig(proxyConfig).build();
