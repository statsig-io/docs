...
This is particularly valuable for SSG (Static Site Generation) or static pages, where gate/config/experiment definitions can be inlined in the payload. While the definitions will be publicly visible, you can synchronously evaluate for an arbitrary set of user properties without requiring additional network requests.
...
- Ideal for SSG/static pages where synchronous evaluation is critical
