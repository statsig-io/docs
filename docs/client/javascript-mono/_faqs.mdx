...
#### Can I reference the SDK instance globally?  What if I initialized using the Javascript snippet, how can I get a reference to the initialized instance?

You can get a reference to the initialized instance via `window.__STATSIG__.instance()`.  

For example, if you initialize using a script tag in the head of your webpage, you can use the following to log a custom event.

```tsx
window.__STATSIG__.instance().logEvent("test_event")
```
