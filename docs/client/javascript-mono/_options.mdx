...
> - 'browser-only' - Only logs events in browser environments (default behavior)
> - 'disabled' - Prevents collecting and sending any events over the network
> - 'always' - Enables logging in all environments (browser and non-browser like VSCode extensions, Chrome extensions, CLIs, etc.)
>
...
#### **logEventCompressionMode** `string`
> Controls whether Statsig should compress JSON bodies for network requests. Options are:
> - 'e' - Enabled (default behavior)
> - 'd' - Disabled
> - 'f' - Forced (compresses even when specifying an API override/proxy)
>
> default: `'e'`
