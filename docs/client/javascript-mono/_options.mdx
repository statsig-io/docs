...
>
> When used with `networkConfig.preventAllNetworkTraffic: true`, this option also prevents the "Failed to flush events" warnings that would otherwise appear in the console, since no events are collected in the first place.
