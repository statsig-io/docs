...
:::note
`useGateValue` and `useFeatureGate` will log an exposure on render. `useStatsigClient.checkGate` will log an exposure when the function is called.
:::
...
:::note
`useDynamicConfig` will log an exposure on render. `useStatsigClient.getDynamicConfig` will log an exposure when the function is called.
:::
...
### Parameter Store Hooks {#parameter-stores}

- (recommended): `useStatsigClient.getParameterStore` will trigger an exposure only when calling `.get()` on a parameter
- `useParameterStore` is a convenience hook that returns the parameter store

:::note
Both approaches only log exposures when calling `.get()` on a parameter, not when the hook is called.
:::
...
function MyComponent() {
  ...
  // Disable exposure logging if needed
...
For more information about Parameter Stores, see [Parameter Stores Concepts](/client/concepts/parameter-stores).
...
:::note
You do not need to use the provided hooks to initialize a `StatsigClient`, and instead can opt to write you own initialization logic. See [Initialization Strategies](/client/javascript-sdk/init-strategies/).
:::
