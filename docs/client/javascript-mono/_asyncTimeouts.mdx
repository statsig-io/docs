...
To target based on a cookie (for logged in users, for example) or based on a UTM parameter (or any query string parameter), you can do so by config a [Custom Field Targeting Rule](https://docs.statsig.com/feature-gates/conditions/#custom) in Statsig Console. This can be configured directly as an _inline experiment targeting rule_ on an experiment — or — as a feature flag condition.
