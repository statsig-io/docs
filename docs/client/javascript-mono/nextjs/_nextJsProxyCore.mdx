...
import { StatsigUser } from "statsig-node";  //Note, recommended page-router implementations still use our legacy SDK.
...
You'll also need to make a small change to add the LogEvents function in the backend:
...
import type { NextApiRequest, NextApiResponse } from 'next';
import { LogEventObject } from 'statsig-node'; //Note, recommended page-router implementations still use our legacy SDK.
