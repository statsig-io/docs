...
```bash
npm install @statsig/next //OR
yarn add @statsig/next //OR
...
export default function RootLayout({
  ...
  return (
    <html lang="en">
      <body>
        <StatsigBootstrapProvider user={user} clientKey={process.env.NEXT_PUBLIC_STATSIG_CLIENT_KEY} serverKey={process.env.STATSIG_SERVER_KEY}>
          {children}
...
```bash
npm install @statsig/react-bindings @statsig/web-analytics //OR 
yarn add @statsig/react-bindings @statsig/web-analytics //OR
