...
export async function generateBootstrapValues(user: StatsigUser): Promise<string> {
  const user = new StatsigUser({ userID: 'userid-123', customIDs: {} }); // Insert your real userID here to be passed 
  await initialize;
...
export default async function RootLayout({
  ...
  const statsigUserObject = StatsigUserCore(user)
  const bootstrapValues = await generateBootstrapValues(statsigUserObject);

  return (
    <html lang="en">
      <MyStatsig user={UserObject} values={bootstrapValues}> //add the user object now to these props
        <body>
          {children}
        </body>
      </MyStatsig>
