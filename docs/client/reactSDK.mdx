...
- the addition of an `initializeValues` prop, which takes in the `initializeValues` generated by the `stasig-node` server SDK method `getClientInitializeValues` introduced in `v4.13.0` of the `statsig-node` sdk ([release notes](https://github.com/statsig-io/node-js-server-sdk/releases/tag/v4.13.0)).
