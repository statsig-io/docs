...
#### Hooks
The SDK exposes Feature Gates, Dynamic Configs, Experiments, and the underlying statsig-js sdk.  If you are unfamiliar with hooks, be sure to check out the [rules of hooks](https://reactjs.org/docs/hooks-rules.html)
...
##### `useStatsig`
A react hook to get an initialized reference to the statsig-js sdk, or undefined if not initialized.  Use this to log events or instead of `useGate`/`useConfig`/`useExperiment` if you are violating the [rules of hooks](https://reactjs.org/docs/hooks-rules.html).  For example, to check gates in a loop, `useStatsig` to get a reference to the sdk, and call `checkGate` in the loop.
