...
- **evaluationCallback**: EvaluationCallbackData, default `nil`
  - EvaluationCallbackData is a callback when an evaluation is made against one of your configurations
  ...
  - e.g. To use the EvaluationCallback, you need to provide a callback function during the SDK initialization via the StatsigOptions. The callback is invoked every time an evaluation occurs for feature gates, dynamic configs, experiments, layers, or parameter stores.

  Here's an example of how to set up an evaluation callback:
  ```
      let opts = StatsigOptions(evaluationCallback: { data in
         switch data {
         case .gate(let gate):
            // Handle gate evaluation
            print("Evaluated gate: \(gate.name), Value: \(gate.value)")
         case .config(let config):
            // Handle dynamic config evaluation
            print("Evaluated config: \(config.name)")
         case .experiment(let experiment):
            // Handle experiment evaluation
            print("Evaluated experiment: \(experiment.name)")
         case .layer(let layer):
            // Handle layer evaluation
            print("Evaluated layer: \(layer.name)")
         case .parameterStore(let paramStore):
            // Handle parameter store evaluation
            print("Evaluated parameter store: \(paramStore.name)")
         }
      })
