...
### 2. Checking an Experiment
 ...
 - The client SDK automatically flushes all accumulated exposure checks events to the Statsig servers every 10 seconds

 -	You can also force this flush by calling `shutdown` to push these accumulated exposure checks to Statsig servers; the `shutdown` call also resets the state of the SDK and you can call it when the application exits
