...
- The client SDK's `initialize` call takes the **client SDK key** and a [**StatsigUser**](/client/concepts/user) object and makes a network request to Statsig servers; this network call fetches precomputed configuration parameters for the specified user from Statsig servers and stores these parameters in local storage on the client device
...
-	The client SDK automatically flushes all accumulated exposure checks and logged custom events to the Statsig servers every 10 seconds; you can also force this flush by calling `shutdown` to push these accumulated exposure checks and custom events to Statsig servers 
