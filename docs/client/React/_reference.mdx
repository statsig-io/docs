...
<FeaturePillList
    features={[
      { name: "Feature Gates", link: "#usegate" },
      { name: "Experiments", link: "#useexperiment" },
      { name: "Dynamic Configs", link: "#useconfig" },
      { name: "Layers", link: "#uselayer" },
...
- `options?: useParameterStoreOptions` - optional evaluation options

Returns `ParameterStore`:

- `name: string` - the name of the parameter store
- `details: EvaluationDetails` - evaluation details including reason and time
- `get: <T>(key: string, fallback?: T) => T` - method to retrieve parameter values with type safety
