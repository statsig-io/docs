...
```kotlin
// Persistent assignment in Android is implemented using the keepDeviceValue parameter
// This ensures users consistently receive the same variant throughout an experiment

// Example using experiment
val experiment = Statsig.getExperiment("new_user_promo", keepDeviceValue = true)
val promoTitle = experiment.getString("title", "Default Title")
...
```
  ...
  <TabItem value="ios">
```swift
// Persistent assignment in iOS is implemented using the keepDeviceValue parameter
// This ensures users consistently receive the same variant throughout an experiment

// Example using experiment
let experiment = Statsig.getExperiment("new_user_promo", keepDeviceValue: true)
let promoTitle = experiment.getValue(forKey: "title", defaultValue: "Default Title")

// Example using layer
let layer = Statsig.getLayer("user_promo_experiments", keepDeviceValue: true)
