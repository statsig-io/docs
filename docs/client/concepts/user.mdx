...
| Country | 2-letter country code of the user. This can be supplied or inferred, and we can target based on the country code in both cases. | `country` | `US` | All | ✔ |
| Locale | Locale of the user. When using the Android or iOS SDK, this will be inferred if not provided. | `locale` | `en_US` | Mobile | ✔ |
| App Version | Version of the app the user is using. When using the Android or iOS SDK, this will be inferred if not provided. | `appVersion` | `1.0.1` | Mobile | ✔ |
...
| Private Attributes | Dictionary that can contain key/value pairs that can be used for Feature Gate targeting. The content of this dictionary will **not** be stored after being used for targeting and will be removed from any `log_event` calls. | `privateAttributes` | `{sensitive_field: "sensitive_information", ...}` | All |  |
