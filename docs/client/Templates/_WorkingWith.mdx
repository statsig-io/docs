...
### Checking a Feature Flag/Gate {#feature-gates}

Now that your SDK is initialized, let's check a [**Feature Gate**](/feature-gates/overview). Feature Gates can be used to create logic branches in code that can be rolled out to different users from the Statsig Console. Gates are always **CLOSED** or **OFF** (think `return false;`) by default.

<div>{props.checkGate}</div>

### Reading a Dynamic Config {#dynamic-configs}

Feature Gates can be very useful for simple on/off switches, with optional but advanced user targeting. However, if you want to be able send a different set of values (strings, numbers, and etc.) to your clients based on specific user attributes, e.g. country, **Dynamic Configs** can help you with that. The API is very similar to Feature Gates, but you get an entire json object you can configure on the server and you can fetch typed parameters from it. For example:

<div>{props.getConfig}</div>

### Getting a Layer/Experiment {#layers}
...
### Logging an Event {#event-logging}
