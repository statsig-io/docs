...
It is a Client SDK that behaves more like a Server SDK. Rather than require a user up front, you can check gates/configs/experiments for any set of user properties, because the SDK downloads a complete representation of your project and evaluates checks in real time.
</div> 
...
- Lower latency to download configs which are cached in Cloudflare CDN at the edge, rather than evaluated on demand for a given user (which cannot be cached as much)
