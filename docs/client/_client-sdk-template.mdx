...
:::info
These docs are for using our javascript SDK in a single-user, client side context. For server side and multi-user contexts, try one of our server SDKs instead.
:::
...
Besides `userID`, we also have `email`, `ip`, `userAgent`, `country`, `locale` and `clientVersion` as top-level fields on StatsigUser. In addition, you can pass any key-value pairs in an object/dictionary to the `custom` field and be able to create targeting based on them.
