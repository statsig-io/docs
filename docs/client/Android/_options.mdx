...
- **customCacheKey**: `((String, StatsigUser) -> String)`, default: `{ sdkKey, user -> "${user.getCacheKey()}:$sdkKey" },` v4.26.0+
  - By default, client SDKs define a "user" for caching as the set of all IDs (the UserID and CustomIDs)
