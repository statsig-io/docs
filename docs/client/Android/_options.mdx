...
- **evaluationCallback**: `((config: BaseConfig) -> Unit)`, v4.24.0+
  - A function that can be supplied to the sdk that will be called whenever a gate, config, experiment or layer is checked. The function takes a BaseConfig, which will be one of FeatureGate, DynamicConfig, Layer, as the only parameter which is the result of the check made.
