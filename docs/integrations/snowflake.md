...
### Setup 
Insert `user_name` and ` user_password` values to the below and run it in a Snowflake worksheet
in an account which has sysadmin and securityadmin roles. 
...
### Sharing Credentials
Go to [console.statsig.com](https://console.statsig.com/) and log in. Go to the settings page and navigate to the integrations tab. 
Find Snowflake in the integrations list and provide the requested credentials. This should the user you created above, which has
...
| timeuuid       | A unique timeuuid for the event     | This should be a timeuuid, but using a unique id will suffice. If not provided, we will generate a UUID. |
...
Make sure these fields are provided where they exist, and are capitalized correctly. Not providing an
ID will limit the utility of metrics, as we won't be able to provide metrics like daily event users.
