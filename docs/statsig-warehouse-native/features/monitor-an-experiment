...
To monitor the status of your experiment, 
 ...
 - **Experiment Health Checks** show alerts for problems with the experiment setup.  Hover over the icon and click on a check for more details.
   ...
   ![Screen Shot 2023-11-09 at 11 11 07 AM](https://github.com/statsig-io/docs/assets/135062389/375aa506-5dbf-42e5-b238-280c907f499b)
    ...
    - **Exposures found** (analysis only) verifies that exposures for this experiment are provided.

    Some health checks are used in experiments where id types are stiched: for example in a sign-up flow experiment where stitching Stable ID to User ID allows for analysis before and after a user is logged in.

    Duplicate mapped IDs happen when the same primary ID is accosicated with multiple secondary IDs or vice versa. This happens when the primary identifier gets reset, or if there are mutliple conversions from the same primary identifier. Deduplication can reduce your sample size and may impact the overall magnitude of your results.
...
The diagnostics page also offers metric alert monitoring
...
You can view instructions for setting up a metric alert [here] (/metrics/metric-alerts)