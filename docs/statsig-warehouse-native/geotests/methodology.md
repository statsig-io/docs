...
2. Apply a known treatment in some "treatment" geographies, and no treatment in some "control" geographies
3. Use the control geographies to figure out what would have happened in the treatment geographies had no treatment been applied (i.e. "synthetic" results)
4. Measure the delta in the treatment geographies between the actual and synthetic values
...
Geotesting relies on the idea that among some sets of geographies there are enough correlations between control and test geographies that make predicting this "counterfactual" possible. We assume, for instance, that general trends and seasonality tend to be shared across some geographies.
...
But what happens when we run a campaign or make some change in just some of these geographies? How can we tell if a fluctuation in this graph is random noise or real stat-sig result? 
...
- Split your geographies into test and control groups
- Using the pre-treatment period data, train a model on the control data to predict how the treatment geographies behave
- Use the post-treatment period data for the control geographies, create a "synthetic control" dataset to predict how the treatment geographies would have behaved after the treatment event.
- Subtract the synthetic data from the actual observed data to determine the induced effects of your treatment on the treatment geographies
...
![Subtracting the modeled Synthetic Control values from the observed Treatment values reveals any incremental effect for the Treatment geographies.](/img/geotests/Incremental_Effects.png)

Subtracting the modeled Synthetic Control values from the observed Treatment values reveals any incremental effect for the Treatment geographies.
