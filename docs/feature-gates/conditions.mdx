...
### Stability
Evaluations at a given percentage are *stable* with respect to the userID.  If userID = 4 passes a condition at a 50% rollout, they will always pass at that 50% rollout. Want to reset that stability? See [Resalting](https://docs.statsig.com/feature-gates/conditions#resalting).
...
If you want to get [Pulse Results](/pulse) (metric movements caused by a feature), simply specifying a number between 0% and 100% will create a random allocation of users in Pass/Fail or "test"/"control" groups for a simple A/B test.
