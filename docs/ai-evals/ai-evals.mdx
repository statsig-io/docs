...
## What is the suite of tools Statsig has for AI Evals?
AI Evals have a few core components to help iterate and serve your LLM apps in production. 
...
4. **Gates, Experiments and Analytics**: The standarad suite of Statsig product building capabilities are also available for use in this scenario. You can for example, target an  LLM feature at a set of people that meet some criteria with a Feature Gate, or choose to roll out a new prompt version as an Experiment and understand impact on metrics, similar to any other experiment.
