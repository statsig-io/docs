...
Create two dynamic configs, one named `gpt-4o` and another named `gpt-4o-mini`.  In the **Value** section add the endpoint, key and other default paramters like this:
...
Navigate to the **Metrics Catalog** page (https://staging.console.statsig.com/metrics/metrics_catalog) and click on **Create** button.
...
The above is just a simple experiment to test models against each other. You could also tweak other parameters like _templerature_, _frequency_penalty_, _max_tokens_, etc. but modifying the config.  This could all be done without needing to update code.
