...
The above is just a simple experiment to test models against each other. You could also tweak other parameters like _templerature_, _frequency_penalty_, _max_tokens_, etc. by modifying the config.  This could all be done without needing to update code.
