...
User objects are also needed for experiment analysis - users are allocated to the test or control group by an ID, and the Statsig stats engine uses the same IDs on the users' events to determine the difference in metrics between test and control. Adding user attributes looks different across Client and Server SDKs:
