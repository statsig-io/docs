...
Requiring pagination parameters to API list endpoints
...
In order to improve the efficiency and performance of our API endpoints, we are requiring pagination parameters to all API endpoints that return a list. This change will make it easier for our customers to manage and navigate through the data returned by these endpoints. This change will also help us maintain the speed and reliability of our API as our platform continues to grow.

## How

Starting from August 1, 2024, all list requests should include the pagination parameters `page` and `limit`. If no parameters are provided, the request will default to `page=1&limit=100`. This change will affect the following endpoints among others: `/gates`, `/metrics`, `/experiments`...
