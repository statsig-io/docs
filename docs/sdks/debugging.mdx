...
In newer server SDKs, the reasons will be a combination of initialization souce and evaluation reason:
| Initialization Sources | Evaluation Reason |
|------------------------|-------------------|
| Network                | LocalOverride     |
| Bootstrap              | Unrecognized      |
| DataAdatper            | Unsupported       |
| StatsigNetwork         | error             |
| Uninitialized          |                   |

- `StatsigNetwork`: fetched from statsig API, most likely due to fallback behavior
