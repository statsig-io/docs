...
<Tabs groupId="evalReasons">
    ...
    |------------|-------------|-------------------|
    | LCUT (Last Config Updated Time) | LCUT is the last time a configuration change was made in the Statsig console - this field represents the last config change that this the SDK is operating with. | Javascript (incl. React, RN), iOS, Dart. |
