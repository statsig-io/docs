...
|Evaluation Performance | Evaluations are done real-time, without a network request. Very complex configurations can take longer to compute, but in pratice, this is rarely an issue.|As all evaluation is precomputed, gate and experiment checks are effectively a dictionary lookup with some computation used for creating and flushing exposure events|
...
|Checking an Experiment| Requires a user object which is evaluated locally (without a network request) against a ruleset persisted in memory | Does not require a user object, uses a dictionary lookup for values fetched during intialize() |
...
|Logging Events| Requires a user object | Does not require a user object. Note, there is some risk of adblocking log events on client SDKs, which can be minmized by setting up a [Custom Proxy](../custom_proxy)|
|Flushing Event| Batched and flushed by the SDK every 60 seconds | Batched and flushed by the SDK every 10 seconds
|Updating Configuarations| Poll Statsig servers for updates every 10 seconds by default (configurable), Streaming possible with some Server SDKs and the [Statsig Forward Proxy](/server/concepts/forward_proxy) | Configuration persists until next `initialize` or `updateUser` call, recommended to call `initialize` at the start of each user session|
