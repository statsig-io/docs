...
- **`enable_country_lookup`**  `Optional[bool]`
  Default off. When enabled, the SDK will attempt to parse IP addresses (attached to the user object at user.ip) into Country codes at evaluation time, when needed for evaluation.
