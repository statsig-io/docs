...
- **`eventLoggingMaxPendingBatchQueueSize`**: `Optional[int]`  
  Maximum number of batches that can be queued before dropping events.
...
- **`serviceName`**: `Optional<String>`  
  A string identifier for the service using the SDK. Used for logging and metrics purposes.
