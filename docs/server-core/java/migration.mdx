---
title: Migrating from Legacy Java SDK to Java Core
...
- **New Features**: Access to Parameter Stores, CMAB (Contextual Multi-Armed Bandits), and more
...
dependencies {
    implementation 'com.statsig:java-server-sdk:X.X.X'
}
```
...
<Tabs
  ...
  defaultValue="legacy"
  values={[
    {label: 'Legacy Java SDK', value: 'legacy'},
...
options.rulesetsSyncIntervalMs = 10 * 1000;
options.idListsSyncIntervalMs = 60 * 1000;
options.disableAllLogging = false;
```

### Java Core SDK Options
...
1. **Update Dependencies**
   - Replace `com.statsig:java-server-sdk` with `com.statsig:javacore`
