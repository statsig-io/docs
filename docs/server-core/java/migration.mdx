...
Since version 0.4.0, Java Core also supports Java 8, which makes it compatible with a broader range of applications. Additionally, you can use the new "uber" JAR that includes native libraries for all supported platforms:
...
```groovy
dependencies {
    implementation 'com.statsig:javacore:X.X.X:uber'  // Self-contained uber JAR with core library and all native libraries
}
```
