...
- **`enableUserAgentParsing`** `Optional<bool>`
  Default off. When enabled, the SDK will attempt to parse UserAgents (attached to the user object) into browserName, browserVersion, systemName, systemVersion, and appVersion.

- **`enableCountryLookup`**  `Optional<bool>`
  Default off. When enabled, the SDK will attempt to parse IP addressed (attached to the user object) into Country codes.
