...
// Get experiment configured for specific group
const groupExp = statsig.getExperimentByGroupName(user, "pricing_experiment", "premium_group");
const premiumPrice = groupExp.getValue("price", 9.99);
const premiumFeatures = groupExp.getValue("advanced_features", false);
```
