...
- **[.NET](./dotnetCoreSDK)**: Stable, [NuGet](https://www.nuget.org/packages/Statsig.Dotnet/0.9.3-beta.2509250234)
- **Ruby**: In Development, planned for Late 2025
- **[Go](./go-core)**: Beta

### Breaking Changes

#### Version 0.10.0: User Agent Parser Change

Starting in v0.10.0, the default user agent parser has changed from a third-party library to a new experimental parser. The new parser is **faster and uses significantly less memory** while maintaining compatibility with most use cases.

**What changed:**
- The default value for user agent parsing now uses the experimental parser
- The experimental parser provides better performance and reduced memory footprint

**Migration:**
If you need to continue using the previous third-party user agent parser, you can opt back in by setting the `__experimental_ua_parsing_enabled` option to `false` in your `StatsigOptions`:
...
**Note:** The configuration option name begins with double underscores (`__experimental_ua_parsing_enabled`) to indicate it's an advanced option. Most users should not need to change this setting and will benefit from the improved performance of the new parser.
