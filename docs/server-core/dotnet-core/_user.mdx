...
- **IP**: User's IP address for geo-targeting
- **UserAgent**: Browser user agent string
- **Country**: ISO country code (e.g., "US", "CA")
- **Locale**: Locale string (e.g., "en-US", "fr-CA")
- **AppVersion**: Application version string
- **CustomIDs**: Additional user identifiers for targeting
- **CustomProperties**: Public user attributes used in targeting rules
- **PrivateAttributes**: Private user data not used in targeting (for logging only)
