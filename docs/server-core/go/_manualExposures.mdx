...
paramValue = layer.Get(aUser, "param_name")`}
  ...
  parameterStoreSnippet: (
    <CodeBlock language="jsx">
      {`paramStore := statsig.GetParameterStore(aUser, "a_parameter_store_name", &statsig.ParameterStoreOptions{DisableExposureLogging: true})
value := paramStore.GetString("a_parameter_name", "default_value")`}
    </CodeBlock>
  ),
};
