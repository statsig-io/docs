...
param_store = statsig.GetParameterStore(*user, "paramStore", options)
```
...
```go
# String parameters
stringVal := paramStore.GetStringFromParameterStore("string_gated_value_param", "default_value")
...
objectVal := paramStore.GetMap("object_gated_value_param", objectParam)

arrayParam := []interface{}{"val1", "val2", "val3"}
arrayVal := paramStore.GetInterface("array_gated_value_param", arrayParam)
