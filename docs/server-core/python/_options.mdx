...
- **`data_store`**: `Optional[DataStoreBaseClass]`  
...
- **`config_compression_mode`**: `Optional[str]`  
  Compression mode for configuration data. Possible values: "gzip", "dictionary".
...
The `ProxyConfig` class is used to configure proxy settings for network requests:

```python
from statsig_python_core import ProxyConfig, StatsigOptions

# Initialize ProxyConfig with custom parameters
