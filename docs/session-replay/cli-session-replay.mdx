...
const client = new StatsigClient(
  ...
  { userID: 'user-123' },
  {
    loggingEnabled: 'always', // Required for CLI environments to enable session replay logging
...
:::note CLI Logging Requirement
CLI applications must set `loggingEnabled: 'always'` when initializing the StatsigClient. By default, Statsig only enables logging in browser environments, but CLI session replay requires logging to be enabled in all environments to capture and send session data.
:::
