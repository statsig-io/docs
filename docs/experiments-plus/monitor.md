...
- **Experiment Health Checks** show alerts for problems with the experiment setup. Hover over the icon and click on a check for more details.
  ...
  - **Crossover units detected** checks for a high percentage (over 0.01%) of units that were exposed to more than one variant. These units are considered invalid for pulse metric calculation as the influence of the experiment on them cannot be attributed to one group. We include them in the group-level statistics as they don't introduce bias generally, if the behavior of crossing over happens across the board.
