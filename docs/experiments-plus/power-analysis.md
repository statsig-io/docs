...
**Note:** This calculation relies on statsitcs computed across the entire user base of the project.  It does not account for the fact that experiments targeting only a subset of users may have different summary statsitics for their key metrics.  For example, the metric mean and variance can be differente in an experiment that targets only Android users or one that exposes users at the lower part of an acquisition funnel. 
