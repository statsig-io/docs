...
## Shipping with a Holdout
Shipping with Holdout

Shipping with holdout lets you release an experiment variant (the “shipped” group) to most of your users while keeping a percentage in the control group for ongoing comparisons. Here’s how it works:

1. Make a decision and select a group to ship: From the Make Decision dropdown, choose which variant you want to ship.
2. Turn on the Ship with Holdout option and specify the percentage of users you want to remain in the control (holdout) group.

![image](https://github.com/user-attachments/assets/0f45e69f-d48b-4608-9473-9ade66e93cab)

3. Allocation of users:
        The control (holdout) group is set to your specified percentage.
...
4. Splitting the shipped group:
        ...
        The Test segment matches the same percentage allocation as the holdout group, allowing a direct head-to-head comparison with control.
        The Launched segment no longer appears in pulse results, but continues to receive the shipped experience.

![image](https://github.com/user-attachments/assets/3299d15c-f777-42a4-9b32-fcd0be0d7e5d)

5. Continue monitoring the pulse results evaluating the Test segment vs. Control (holdout)
...
By using shipping with holdout, you maintain a dedicated, stable control group alongside a representative test segment of the shipped experience, making it easy to measure ongoing performance and user behavior post-launch.
