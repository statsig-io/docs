...
One common misconception when working with experiments on Statsig is trying to check the experiment group in code.  Checking the experiment group _in code_ is actually an anti-pattern.  Its not necessary with Statsig, and it will limit your ability to quickly test different variants.
...
async function getSearchItems(user: StatsigUser, searchTerm: String): String[] {
    ...
    const experiment = await statsig.getExperiment(user, 'search_results');
    // These are the actual APIs you can use
