...
The easiest way to run an A/A test in Statsig is by leveraging a [Feature Flag](https://docs.statsig.com/feature-flags/working-with). You can also leverage an [Experiment](https://docs.statsig.com/guides/abn-tests) to run an A/A, but we chose to use a Feature Gate for this tutorial for simplicity. 
