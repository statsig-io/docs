...
The easiest way to run an A/A test in Statsig is by leveraging a [Feature Flag](/feature-flags/overview). You can also leverage an [Experiment](/guides/abn-tests) to run an A/A, but we chose to use a Feature Gate for this tutorial for simplicity. 
