...
Email campaigns are a critical tool for any Marketing team.  And finding the best performing Email template is a perfect use-case for an A/B test.  Statsig allows you to run simple but powerful A/B tests on different parts of your email content.  And since Statsig can integrate seamlessly with product analytics, you can run email experiments and understand deeper business level impact easily.
...
By going to the **Pulse Results** tab in the Experiment page, you can add metrics you want to monitor and verify which variant is doing better.  To learn more about reading Pulse Results, check this article out: [Reading Pulse Results](../pulse/read-pulse.md).
