...
User dimensions let you slice or filter metrics based on a user property. This is often something like Country, Device Type or a property like Free vs Paid. On Statsig Cloud, theser are frozen when a user is first exposed to a feature gate or experiment - in case your experiment ends up changing these properties (e.g. convert a Free user to Paid). If you're using Statsig Warehouse Native, these properties can be set anytime before analysis. 

## Slack Notifs
Statsig supports two classes of Slack Notifcations -  Project and Personal Notifications. [See more](https://docs.statsig.com/integrations/slack)
