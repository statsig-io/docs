...
Statsig provides various approaches for tracking events when using Sidecar. You can use the built-in "Autocapture" tracking that comes with Sidecar or set up custom event logging. For detailed guidance on autocapture events, refer to the [Web Analytics Events documentation](/webanalytics/events). For sidecar-specific tracking methods, refer to the [sidecar measuring experiments documentation](/guides/sidecar-experiments/measuring-experiments).
