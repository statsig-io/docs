...
You can implement a device-level experiment almost exactly like a [traditional user-level experiment](./user-level-experiments.mdx). The key difference is setting the experiment’s ID type to **Stable ID**.
...
After setting up your experiment in the Statsig console, the next step is to integrate it into your client application using one of [Statsig's SDKs](../sdks/client-sdks-overview.mdx).
...
> **Tip**: If your app collects other relevant attributes (e.g., device type, region), pass them in the `user` object to improve experiment precision. Learn more about [user attributes](./user-attributes.mdx).
...
For more information on experiments, check out our [Experiments Overview](../experiments/experiments-overview.mdx) guide.
