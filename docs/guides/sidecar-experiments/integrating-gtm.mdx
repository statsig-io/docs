---
sidebar_label: Integrating with GTM
title: Integrating with GTM for Event Tracking
---
...
### Step 2: Choose "Custom HTML" for tag type
Paste [this GTM code](#gtm-code) (including script tag)
...
window.StatsigLogger = (function () {
  ...
  var checkStatsigReady = function (callback) {
    var checkerLoop, idx = 1,
      killLoop = function () {
        log('+++ Stop checkStatsigReady after ' + idx + ' checks');
        try {
          clearTimeout(checkerLoop);
        } catch (err) { }
      };
    (function checkerFcn() {
      if (idx >= 10) {
      ...
      try {
        log('+++ Check num: ' + idx);
        if (StatsigSidecar.getStatsigInstance().ready) {
          log('+++ Statsig Ready!');
          ready = true;
          statsigInstance = StatsigSidecar.getStatsigInstance();
          killLoop();
          callback();
          return;
        }
      ...
      idx++;
      checkerLoop = setTimeout(checkerFcn, 250);
    })();
  };

  var track = function (eventName, eventValue, eventMetadata) {
    log('+++ Track', eventName, eventValue, eventMetadata);
    if (ready) statsigInstance.logEvent(eventName, eventValue, eventMetadata);
    else enqueuedEvents.push([eventName, eventValue, eventMetadata]);
  }

  var flushQueuedEvents = function () {
    log('+++ Flushing queued events', JSON.stringify(enqueuedEvents));
    for (var i = enqueuedEvents.length - 1; i >= 0; i--) {
      log('+++ Flush', enqueuedEvents[i]);
      statsigInstance.logEvent.apply(statsigInstance, enqueuedEvents[i]);
      enqueuedEvents[i].splice(i, 1);
    }
  }
  ...
  var handleGTMMessage = function (model, message) {
    if (typeof message === 'object' && typeof message.event === 'string') {
      ...
      track(message.event, message.conversionValue || null, metadata);
    }
  }

  var debug = function () {
    log('+++ Debug', ready, enqueuedEvents);
  }

  checkStatsigReady(function () {
    flushQueuedEvents();
    // this may have past messages
  ...
  return { track: track, debug: debug };
})();
</script>
```