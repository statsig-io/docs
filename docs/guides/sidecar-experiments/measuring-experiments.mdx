...
<script>function getUUID(){if(crypto&&crypto.randomUUID)return crypto.randomUUID();let t=()=>Math.floor(65536*Math.random()).toString(16).padStart(4,"0");return`${t()}${t()}-${t()}-4${t().substring(1)}-${t()}-${t()}${t()}${t()}`}function setUUICookie(t){var a=new Date;a.setMonth(a.getMonth()+12);var o=window.location.host.split(".");o.length>2&&o.shift();var s=`.${o.join(".")}`;document.cookie=`statsiguuid=${t||getUUID()};Expires=${a};Domain=${s}`}var statsigUUIDCookieVal=null,statsigUUIDLocalStorageVal=localStorage.getItem("STATSIG_LOCAL_STORAGE_STABLE_ID")||null;if(document.cookie.match(/statsiguuid=([\w-]+);?/))var[,statsigUUIDCookieVal]=document.cookie.match(/statsiguuid=([\w-]+);?/);if(statsigUUIDCookieVal&&statsigUUIDLocalStorageVal&&statsigUUIDCookieVal===statsigUUIDLocalStorageVal);else if(statsigUUIDCookieVal&&statsigUUIDLocalStorageVal&&statsigUUIDCookieVal!==statsigUUIDLocalStorageVal)localStorage.setItem("STATSIG_LOCAL_STORAGE_STABLE_ID",statsigUUIDCookieVal);else if(statsigUUIDCookieVal&&!statsigUUIDLocalStorageVal)localStorage.setItem("STATSIG_LOCAL_STORAGE_STABLE_ID",statsigUUIDCookieVal);else{var t=getUUID();localStorage.setItem("STATSIG_LOCAL_STORAGE_STABLE_ID",t),setUUICookie(t)}</script>
