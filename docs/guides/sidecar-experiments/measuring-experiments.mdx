...
Auto capture will automatically add data attributes to the event metadata object. This is available for `click` events only for now. The metadata will be in the format of data-(camelCasedAttributeName).
