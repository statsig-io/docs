...
* **Prerun Script** - This is a script that will run only once after Sidecar initialization (only if experiment URL filters pass). You should use this to bind any listeners or evaluate any custom logic you need to control when to trigger the experiment using `activateExperiment`.
