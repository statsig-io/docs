...
* **Prerun Script** - This is a script that will run on _any page_ where Sidecar is loaded (regardless of URL filters). You should use this to bind any listeners or evaluate any custom logic you need to control when to trigger the experiment using `activateExperiment`.
