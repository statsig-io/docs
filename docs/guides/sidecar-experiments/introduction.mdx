...
Sidecar (along with any type of client side tooling you install, be it open-source or via experimenation providers) will introduce some degree of page load latency. We recommend taking Sidecar for a spin and determining if the impact of your performance metrics fall within your acceptable threshold.

Customers that are very performance-minded typically use our [JS-SDK](client/javascript-sdk) for testing on the web. When using the JS/React SDKs with [Client Boostrapping](client/javascript-sdk/init-strategies/#2-bootstrap-initialization), the latency introduced is very minimal since there are no network requests required for initialization.
