...
5. Create [Segments](/segments) to identify internal employees so you can easily override them into feature rollouts and experiments.

## 2. Setup SSO to ease onboarding

Instead of having to invite each user and remembering to deactivate them if they leave your company, set up [Single Sign On](/access-management/introduction) with your identity provider. Statsig supports OIDC - a modern protocol that most identity providers support. Employees (your identity provider verifies) will have a Statsig account created on first use. Once you do this, make your project "Open" so new users verified by SSO default to having access to your projects. If you choose to keep your project closed, you can explicitly approve new requests.
...
You should make sure [metrics](/metrics) and dimensions you want to experiment on are set up and correct before starting experiments. Some things to consider/explore while determining this include

- [Custom Metrics](/metrics/create) that build on top of other metrics (e.g. aggregations, ratios)
- User Dimensions to be able to slice experiment results by Geo, User etc.
- [Metric Dimensions](/metrics#metric-dimensions) to be able to able slice metrics (eg. add-to-cart events by Product Category)
- [Funnels](/metrics/create-user-funnels)
- [Tag metrics](/metrics/create-metric-tags) so that frequently used collections of metrics are easy to bring into experiments

![image](https://user-images.githubusercontent.com/31516123/178082121-bb258851-29e5-4732-bd2a-af87833f7570.png)

Decide if you want to [export data from Statsig](/integrations/data-exports/overview) back into your systems.

Consider if you want to [customize Statsig's DAU definition](/metrics/user#definition-of-a-daily-active-user) to include only meaningful events. The default is to count a user as active if they generate any event.
...
[Read more](/guides/experiment-on-custom-id-types)

## 6. Set up integrations

You might already have set up [integrations](/integrations/introduction) to bring data into Statsig. Consider other integrations that may be helpful -
