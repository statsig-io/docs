...
Most experiments or feature rollouts on Statsig target users (identified by userID). Another identity type Statsig supports is stableID. This is a Statsig client SDK managed ID unique to a device - typically used to experiment on anonomyous users. You can also create arbitrary units of assignment (eg. organizationID for B2Bs, VIN or vehicleIDs if you're controlling features on cars, creatorID if you have a two sides marketplace with creators and consumers (regular users) and want to target both). [Read more](https://docs.statsig.com/guides/experiment-on-custom-id-types).
