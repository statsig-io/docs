---
sidebar_label: CMS Integrations
title: Guide to Hubspot/Contentful/CMS Integrations
---

### Using Statsig with a CMS

One fairly common question we get is around how to use Statsig with an existing CMS.  While we currently do not offer a no-code solution,
...
Note that each layer parameter has a default value.  If the user is not in any experiments in that layer, thats the default they will get, which will be backed by the cms.
...
If you want to create multiple experiment groups, or add more paramters, keep on adding until your experiment setup is complete.
