...
A/B Testing an email campaign and getting experiment results on downstream product metrics (in addition to top level email interaction metrics), is a common use case amogst Statsig customers.

The general flow looks like this:

1. Create an Experiment in Statsig for this email campaign.
2. Check that experiment for each user you are sending the email to.  Record the variant of the email to send.  If you are writing a script to do this, make sure to send the exposure events to Statsig (either `flush()` or `shutdown()` the SDK before exiting the script). If you are unsure, check the Diagnostics page of your experiment and make sure the exposure checks are recorded for each of the users.
