...
8. **Caching on server SDKs**: Initializing Statsig server SDKs requires them to connect to Statsig and download config. If connectivity to Statsig fails, initialization fails (falling back to default values). Do two things to mitigate this risk - 1. Deploy the Statsig [Forward Proxy](https://docs.statsig.com/server/concepts/forward_proxy/) so your servers connect to this endpoint to download config, instead of from Statsig directly. You can also reduce connectivity related SDK initialization failures by providing config locally using a [dataAdapter](/server/concepts/data_store#dataadapter-or-datastore).
