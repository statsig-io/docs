...
If you suspect adblockers, consider setting up a [proxy](/custom_proxy/). For server issues, inspect specific error logs where your Server SDK is running for more information.
...
For Client SDKs:
- Data loss due to adblockers
- Data loss when users are immediately redirected after a "check"

For Server SDKs:
- Process exits before events are sent (typical in serverless situations)
