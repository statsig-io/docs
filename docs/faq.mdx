...
Some edge providers offer utilities to ensure events are flushed without blocking the response to the client. [Learn more](https://docs.statsig.com/server/nodejsServerSDK#flushing-events).
...
For more details, see how we [calculate p-values](https://docs.statsig.com/stats-engine/p-value).

---

## How does Statsig handle low sample size?

For small sample sizes, Statsig uses [Welch's t-test](https://docs.statsig.com/stats-engine/p-value#welchs-t-test) to handle unequal sample sizes or variances while maintaining low false positive rates.
...
Statsig uses **mSPRT** for [sequential testing](https://docs.statsig.com/experiments-plus/sequential-testing). This method dynamically adjusts the confidence interval based on the variance and sample size, ensuring a controlled false positive rate while detecting significant effects early.

---

## What is CUPED?

[CUPED](https://docs.statsig.com/stats-engine/methodologies/cuped) stands for **Controlled-experiment Using Pre-Experiment Data**. It’s a technique that reduces the variance of estimates by using pre-experiment data as a covariate in the analysis. This boosts the speed and accuracy of experiments.
...
Learn more about working with feature flags [here](https://docs.statsig.com/feature-flags/working-with).
...
For more details, check the [Console API](https://docs.statsig.com/console-api/usage-billing) for downloading usage data.
