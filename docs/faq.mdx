...
Bucketing on Statsig is deterministic. Given the same UserID (or any UnitID) and an experiment/gate a given rollout stage, we'll always evaluate to the same result. This is true even if the evaluation happens in different places - on the client or server. For a peek into how we do this at a high level:
