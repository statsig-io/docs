...
Whenever you call .logEvent() with user properties, we’ll automatically update the user profile. New properties are added, and existing ones are updated as needed. This means you don't need to send the same user properties on every event, and you can simplify .logEvent() calls.
