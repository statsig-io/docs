...
**Target Duration:** The intended length of time to run the experiment. The experiment will **not** automatically stop when this duration is reached. To end the experiment at this point, enable **“Stop Experiment at target duration.”** When the experiment stops, all users will be served the default experiment value configured in code.
...
**Specify Pre-computed User Dimensions:** Configure user dimensions that can be used to break down experiment results in the results section. These fields are selected from the exposure user properties or entity properties.