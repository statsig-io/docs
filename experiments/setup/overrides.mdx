...
For Visual Editor experiments, you can also create a dedicated test user ID such as `preview_test` in **Manage Overrides**, then open your site with `?overrideuser=preview_test` to preview that exact experience. See [Visual Editor Setup & Usage](/guides/sidecar-experiments/sidecar-v3#testing-a-specific-user-with-overrides).
