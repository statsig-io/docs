---
title: Cloudflare KV
description: Statsig offers a set of integrations that make usage with Cloudflare easy: * Automatically pushing changes to Cloudflare's KV store, for low-latency S
---
...
<ResponseField name="statsigOptions" type="StatsigOptions">
  See StatsigOptions [here](https://docs.statsig.com/client/javascript-sdk#statsig-options)
</ResponseField>
...
export default handleWithStatsig(
  async (request, env, ctx, client) => {
    ...
    const gate = client.getFeature Flag("test_cloudflare_sync", { userID: randomUserId });
...
The following line initializes the client by loading feature flag and experiment configurations directly from your Cloudflare KV store.
...
<Frame>
  <img src="/images/integrations/cloudflare/1cc865ed-e15c-41a4-8979-24e1d457a7b1.png" alt="Diagnostics Stream" />
</Frame>
