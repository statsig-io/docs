...
We're asking all customers to make this change by October 31st, 2025. Note - CDN access is only available in a more recent version of the PHP SDK, so we won't strictly enforce an upgrade, but we still recommend you upgrade as soon as possible.
