...
<Note>
These overrides only apply locally—they do not update definitions in the Statsig console or elsewhere. The local override API is not designed to be a full mock; it is only a convenient way to override the value of a gate, config, or other entity.
</Note>
