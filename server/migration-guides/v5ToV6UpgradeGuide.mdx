...
description: "Upgrade Statsig server SDKs from v5 to v6, including breaking changes, API updates, and configuration adjustments needed for the new major version."
---
...
We also embedded the compress encoding format we are using, right now we are only storing plain_text no-compressed version within Cache.
